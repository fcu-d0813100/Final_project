...
router.get('/product-list/category/:main_category_id', async function (req, res, next) {
...
router.get('/product-list/category/:main_category_id/:sub_category_id', async function (req, res, next) {
  const main_category_id = parseInt(req.params.main_category_id, 10); // 從路由參數中取得 main_category_id
  const sub_category_id = parseInt(req.params.sub_category_id, 10); // 從路由參數中取得 sub_category_id
  console.log("Received main_category_id:", main_category_id, "and sub_category_id:", sub_category_id); // 打印確認接收成功
  ...
  try {
    ...
    const [result] = await db.query(sqlSelect, [main_category_id, sub_category_id]);
