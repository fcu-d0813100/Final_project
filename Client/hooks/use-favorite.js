...
export function useFavorite() {
  ...
  const fetchFavorites = async () => {
    try {
      const response = await fetch(`http://localhost:3005/api/product/favorite/search/${auth.userData.id}`);
  ...
  const handleFavoriteClick = async (product) => {
    ...
    try {
      if (favoriteProducts[color_id]) {
        ...
        setFavoritesList(favoritesList.filter((item) => item.color_id !== color_id));
      } else {
